...
def write_predict_to_file(tokens, metaphor_labels, frame_labels, out_file='predictions.csv',):
        
    with open(out_file, 'w', encoding='utf-8') as f:
        f.write('Tokens\tMetaphor_label\tFrame_label\n')
        for token, mls, fls in zip(tokens, metaphor_labels, frame_labels):
            for index_, (t, m, f_) in enumerate(zip(token, mls, fls)):
                line = f'{t}\t{m}\t{frame_list[f_]}'
                f.write(line+'\n')
            f.write('\n')
...
if __name__ == '__main__':
    ...
    metaphor_model, frame_model, = sys.argv[1:]
    ...
    sents = ['The clouds parted and hard gashes of sunlight swooped down to stain the earth with streaks of white and gold light so that the shadows of the running horses flowed like dark streams over the dazzling snow .']
    sents = [word_tokenize(sent) for sent in sents]
    ds = datasets.Dataset.from_dict({'tokens': sents})

    def tokenize(x):
        out = tokenizer(x['tokens'], truncation=True, padding='max_length', is_split_into_words=True)
    ...
    write_predict_to_file(ds['tokens'], metaphor_predictions, frame_predictions, out_file=prediction_output_file)